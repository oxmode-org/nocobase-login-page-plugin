...
test('renders public support text without interpreting it as HTML and preserves NocoBase attribution', () => {
  ...
  assert.match(poweredBy, /PoweredBy as NocoBasePoweredBy/);
  assert.match(poweredBy, /<NocoBasePoweredBy \/>/);
  assert.doesNotMatch(poweredBy, /if \(!supportText\) return null/);
});
...
test('keeps package manifest and lockfile release versions synchronized', () => {
  const pkg = JSON.parse(read('package.json'));
...
test('serves only configured login media through a public allowlist endpoint', () => {
  ...
  assert.match(server, /ctx\.set\('Cache-Control', 'private, no-store'\)/);
  assert.match(server, /ctx\.set\('X-Content-Type-Options', 'nosniff'\)/);
...
test('ignores runtime audit logs generated by NocoBase', () => {
  assert.match(read('.gitignore'), /^storage\/logs\/$/m);
});
